...
from @Lister2009. To load this data directly, simply type `data(BS.chr21)`.
...
In addition, to avoid a block being broken up simply due to a gap with no covered 
CpGs, you can increase the `maxGap` parameter.
...
In general, it also might be advised to decrease the cutoff when detecting blocks, 
since a smaller methylation
...
span `r signif((max(end(bs[120001:125000,])) - min(start(bs[120001:125000,])))/1e3,3)` 
