...
`filterLoci`.  By default, this function will only keep Loci with at least one
read in every sample (note that the zero
...
However, by specifying `numSamples="one"`, it will keep all loci with at least
one read in at least *one* sample per group. You can also change the minimum
number of reads parameter (default `minreads=1`).

```{r filter, message=TRUE}
bs <- filterLoci(bs, numSamples="one", testCovariate = "CellType")
```

You may also change the minimum
number of reads parameter (default `minCoverage=1`).
...
# load the annotatr package & get annotations for hg18
library(annotatr)
