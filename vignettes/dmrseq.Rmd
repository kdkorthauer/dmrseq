...
loci.idx <- which(rowSums(getCoverage(BS.chr21, type="Cov")==0) == 0)
sample.idx <- which(pData(BS.chr21)$CellType %in% c("imr90", "h1"))

BS.chr21.filtered <- BS.chr21[loci.idx, sample.idx]
```
...
and then add __in silico__ DMRs. This can then be used to assess sensitivity
...
simulated DMRs.
list(gr.dmrs = gr.dmrs, dmr.mncov = dmr.mncov, dmr.L = dmr.L,
