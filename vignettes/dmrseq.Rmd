...
abstract: >
  ...
  dmrseq package version: `r packageVersion("dmrseq")`
output:
...
[bsseq](http://bioconductor.org/packages/bsseq) package, which provides
efficient storage and manipulation of bisulfite sequencing data and 
inference for differentially methylated CpGs. The main goal of dmrseq
...
use the `read.bismark` function from bsseq to read the data directly into 
a `BSeq` object. 
...
you should filter them out. This can be done as follows (note that the zero
...
differences mean h1 has higer methylation than imr90 (see below for
more details).
