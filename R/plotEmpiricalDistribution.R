...
#' 
plotEmpiricalDistribution <- function(bs, plotFile, tiss1, tiss2, time = NULL, 
                                      time2 = NULL, matchCovariate = NULL, 
                                      genomeName = NULL) {
    ...
    # plot empirical distribution of meth levels (betas) random sample of
    # 1M rows
    rows.plot <- sample(1:nrow((meth.mat)), min(nrow((meth.mat)), 1e+06), 
                        replace = FALSE)
    ...
    if (!is.null(matchCovariate)) {
        if (sum(grepl(matchCovariate, colnames(pData(bs)))) == 0) {
            stop(paste0("Error: no column in pData() found ", 
                        "that matches the matchCovariate"))
        } else if (length(grep(matchCovariate, colnames(pData(bs)))) > 1) {
            stop(paste0("Error: matchCovariate matches more ",
                        "than one column in pData()"))
        }
        mC <- grep(matchCovariate, colnames(pData(bs)))
        
        if (length(grep(pData(bs)[1, mC], colnames(meth.mat))) == 0) {
            colnames(meth.levelsm) <- paste0(colnames(meth.levelsm), 
                                             "_", pData(bs)[, mC])
            colnames(cov.matm) <- paste0(colnames(meth.levelsm), 
                                         "_", pData(bs)[, mC])
        }
    }
    ...
    pdf(file = plotFile, height = 3, width = 6)
    
    p0 <- ggplot(meth.levelsm, 
                 aes(value, colour = variable, group = variable)) + 
        geom_line(adjust = 2.5, alpha = 0.6, stat = "density", size = 1.3) + 
      xlab("Methylation Proportion") + 
        theme_bw()  # colour=condition
    p1 <- ggplot(cov.matm, aes(log(value), 
