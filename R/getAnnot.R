...
build_cpg_annots2 <- function(genome = builtin_genomes2(), 
    ...
    annot_codes <- data.frame(code = c(sprintf("%s_cpg_islands", genome), 
    ...
    if (genome == "hg19" || genome == "mm9" || genome == "rn5" ||
        genome == "rn4") {
        use_ah <- TRUE
    } else if (genome == "hg38") {
        use_ah <- FALSE
        con <- paste0("http://hgdownload.cse.ucsc.edu/goldenpath/",
                      "hg38/database/cpgIslandExt.txt.gz")
    } else if (genome == "hg18") {
        use_ah <- FALSE
        con <- paste0("http://hgdownload.cse.ucsc.edu/goldenpath/",
                      "hg18/database/cpgIslandExt.txt.gz")
    } else if (genome == "mm10") {
        use_ah <- FALSE
        con <- paste0("http://hgdownload.cse.ucsc.edu/goldenpath/",
        ...
        use_ah <- FALSE
        con <- paste0("http://hgdownload.cse.ucsc.edu/goldenpath/",
                      "rn6/database/cpgIslandExt.txt.gz")
    } else {
    ...
    if (use_ah) {
        ...
        ID <- row.names(GenomicRanges::mcols(query)[
          GenomicRanges::mcols(query)$genome == genome, ])
    }
    
    if (any(grepl("islands", annotations)) || 
        any(grepl("shores", annotations)) || 
        any(grepl("shelves", annotations)) || 
        any(grepl("inter", annotations))) {
        message("Building CpG islands...")
        ### Islands Extract and sort the islands based on use_ah
        if (use_ah) {
            islands <- ah[[ID]]
        } else {
            # Read from URL. There is surprisingly nothing in 
            # base that does this as easily,
            # so here we are with readr again.
            if (genome == "hg18") {
            ...
            # Convert to GRanges
            islands <- GenomicRanges::GRanges(seqnames = islands_tbl$chr, 
                          ranges = IRanges::IRanges(start = islands_tbl$start, 
                end = islands_tbl$end), strand = "*", 
        ...
        # Rename the islands
        GenomicRanges::mcols(islands)$id <- paste0("island:",
                                                   seq_along(islands))
        ...
        GenomicRanges::mcols(islands) <- GenomicRanges::mcols(islands)[, c("id",
        ...
        if (any(grepl("shores", annotations)) || 
            ...
            ### Shores Construct the shores based on: upstream 
            ### from the island start and
            ### downstream from the island end
            ...
            # Rename the shores
            GenomicRanges::mcols(shores)$id <- paste0("shore:", 
                                                      seq_along(shores))
            ...
            # Give it the correct type
            GenomicRanges::mcols(shores)$type <- sprintf("%s_cpg_shores", 
                                                         genome)
            
            GenomicRanges::mcols(shores) <- GenomicRanges::mcols(shores)[, 
                                c("id", "tx_id", "gene_id", "symbol", "type")]
            
            if (any(grepl("shelves", annotations)) || 
                any(grepl("inter", annotations))) {
                message("Building CpG shelves...")
                ### Shelves Construct the shelves based on: upstream from 
                ### the up_shores start and
                ### downstream from the down_shores end
                up_shelves <- GenomicRanges::flank(shores, width = 2000, 
                                                   start = TRUE, both = FALSE)
                down_shelves <- GenomicRanges::flank(shores, width = 2000, 
                                                   start = FALSE, both = FALSE)
                
                # Combine, sort, trim, and reduce combined up_shelves 
                # and down_shelves
                ...
                # Rename the shelves
                GenomicRanges::mcols(shelves)$id <- paste0("shelf:", 
                                                           seq_along(shelves))
                ...
                # Give it the correct type
                GenomicRanges::mcols(shelves)$type <- sprintf("%s_cpg_shelves",
                                                              genome)
                
                GenomicRanges::mcols(shelves) <- GenomicRanges::mcols(shelves)[,
                  c("id", "tx_id", "gene_id", "symbol", "type")]
                
                if (any(grepl("inter", annotations))) {
                  message("Building inter-CpG-islands...")
                  ### interCGI Take the union of all the objects so far
                  extended_cgi <- Reduce(function(x, y) {
                    GenomicRanges::union(x, y)
                  }, list(islands, shores, shelves))
                  
                  # A quirk of GenomicRanges::gaps() on unstranded ranges gives 
                  # the whole chroms on the + and - strands in addition to the
                  # * gaps. Remove +/- gaps.
                  ...
                  # Rename the interCGI
                  GenomicRanges::mcols(inter_cgi)$id <- paste0("inter:", 
                                                          seq_along(inter_cgi))
                  ...
                  # Give it the correct type
                  GenomicRanges::mcols(inter_cgi)$type <-sprintf("%s_cpg_inter",
                    genome)
                  
                  GenomicRanges::mcols(inter_cgi) <- GenomicRanges::mcols(
                    inter_cgi)[,c("id", "tx_id", "gene_id", "symbol", "type")]
                }
            }
        }
    }
...
#'
#' @param genome The genome assembly.
#' @param annotations A character vector of annotations to build. Valid 
#' annotation codes are listed with \code{builtin_annotations2()}. The 
#' 'basicgenes' shortcut builds the following regions: 1-5Kb upstream of TSSs,
#'  promoters, 5UTRs, exons, introns, and 3UTRs. The 'cpgs' shortcut builds the
#'   following regions: CpG islands, shores, shelves, and interCGI regions. 
#'   NOTE: Shortcuts need to be appended by the genome, e.g. 
#'   \code{hg19_basicgenes}. Custom annotations whose names are of the form 
#'   \code{[genome]_custom_[name]} should also be included. Custom annotations
#'    should be read in and converted to \code{GRanges} with
#'     \code{read_annotations()}. They can be for a \code{supported_genome()}, 
#'     or for an unsupported genome.
#'
#' @return A \code{GRanges} object of all the \code{annotations} combined. 
#' The \code{mcols} are \code{id, tx_id, gene_id, symbol, type}. The \code{id} 
#' column is a unique name, the \code{tx_id} column is either a UCSC knownGene
#' transcript ID (genic annotations) or a Ensembl transcript ID 
#' (lncRNA annotations), the \code{gene_id} is the Entrez ID, the \code{symbol}
#'  is the gene symbol from the \code{org.*.eg.db} mapping from the Entrez ID, 
#'  and the \code{type} is of the form \code{[genome]_[type]_[name]}.
#'
#' @examples
...
build_annotations2 <- function(genome, annotations) {
    ...
    builtin_annotations <- c(hmm_annotations, enh_annotations, gene_annotations,
        cpg_annotations, lncrna_annotations)
    ...
    # Do the other_annotations first because we don't want to fail unexpectedly
    # at the end
    if (length(other_annotations) > 0) {
        annots_grl <- c(annots_grl, GenomicRanges::GRangesList(sapply(
          other_annotations, 
            function(ca) {
                annotatr::annotatr_cache$get(ca)
            })))
    }
...
builtin_genomes2 <- function() {
    return(c("dm3", "dm6", "hg19", "hg38", "mm9", 
             "mm10", "rn4", "rn5", "rn6", "hg18"))
}
...
builtin_annotations2 <- function() {
    ...
    gene_ends <- c("1to5kb", "promoters", "cds", "5UTRs", "exons", "firstexons",
        "introns", "intronexonboundaries", "exonintronboundaries",
        "3UTRs", "intergenic")
    ...
    # Chromatin state codes Remove numbers, and underscores, and take unique
    chromatin_recode <- unique(annotatr:::reformat_hmm_codes(
      annotatr:::HMMCODES))
    
    chromatin_ends <- apply(expand.grid(annotatr:::HMMCELLLINES, 
                                        chromatin_recode, 
        stringsAsFactors = FALSE), 1, paste, collapse = "-")
    
    chromatin_shortcut_ends <- apply(expand.grid(annotatr:::HMMCELLLINES,
                                                 "chromatin", 
        stringsAsFactors = FALSE), 1, paste, collapse = "-")
    
    # Create full annotation codes
    gene_codes <- apply(expand.grid(gene_genomes, "genes", gene_ends,
                                    stringsAsFactors = FALSE), 
        1, paste, collapse = "_")
    cpg_codes <- apply(expand.grid(cpg_genomes, "cpg", cpg_ends,
                                   stringsAsFactors = FALSE), 
        1, paste, collapse = "_")
    chromatin_codes <- apply(expand.grid("hg19", "chromatin", chromatin_ends,
                                         stringsAsFactors = FALSE), 
        1, paste, collapse = "_")
    
    enhancer_codes <- c("hg19_enhancers_fantom", "hg38_enhancers_fantom",
                        "mm9_enhancers_fantom", "mm10_enhancers_fantom")
    lncrna_codes <- c("hg19_lncrna_gencode", "hg38_lncrna_gencode",
                      "mm10_lncrna_gencode")
    
    gene_shortcut_codes <- apply(expand.grid(gene_genomes, "basicgenes", 
                                             stringsAsFactors = FALSE), 
        1, paste, collapse = "_")
    cpg_shortcut_codes <- apply(expand.grid(cpg_genomes, "cpgs", 
                                            stringsAsFactors = FALSE), 
        1, paste, collapse = "_")
    chromatin_shortcut_codes <- paste("hg19", chromatin_shortcut_ends,
                                      sep = "_")
    
    # Create the big vector of supported annotations
    annots <- c(gene_codes, cpg_codes, chromatin_codes, 
                enhancer_codes, lncrna_codes, gene_shortcut_codes, 
                cpg_shortcut_codes, chromatin_shortcut_codes)
...
check_annotations2 <- function(annotations) {
    ...
    # Check that the annotations are supported, tell user which are unsupported
    if (!all(annotations %in% builtin_annotations2())) {
        unsupported <- base::setdiff(annotations, builtin_annotations2())
        stop(sprintf(paste0("Error: \"%s\" is(are) not supported. ",
                     "See builtin_annotations2()."), 
            paste(unsupported, collapse = ", ")))
    }
    
    # Recombine annotations and custom_annotations or you get 
    # failure when there are only custom annotations
...
#' modified from annotatr to include hg18
#'
#' Using the \code{TxDb.*} group of packages, construct genic annotations 
#' consisting of any combination of 1-5kb upstream of a TSS, promoters 
#' (< 1kb from TSS), 5UTRs, CDS, exons, first exons, introns, intron/exon and
#'  exon/intron boundaries, 3UTRs, and intergenic.
#'
#' @param genome The genome assembly.
#' @param annotations A character vector with entries of the form 
#'  \code{[genome]_genes_{1to5kb,promoters,5UTRs,cds,exons,firstexons,introns,
#'  intronexonboundaries,exonintronboundaries,3UTRs,intergenic}}.
#'
#' @return A list of \code{GRanges} objects with unique \code{id} of the 
#' form \code{[type]:i}, \code{tx_id} being the UCSC knownGene transcript name,
#'  \code{gene_id} being the Entrez Gene ID, \code{symbol} being the gene symbol
#'  from the Entrez ID to symbol mapping in \code{org.db} for that species, and 
#'  \code{type} being the annotation type.
build_gene_annots2 <- function(genome = builtin_genomes2(), 
    ...
    annot_codes <- data.frame(code = c(sprintf("%s_genes_promoters", genome), 
                    ...
                    var = c("promoters_gr",  "onetofive_gr", "cds_gr", 
                            "fiveUTRs_gr", "exons_gr", "firstexons_gr", 
                            "introns_gr", "intronexon_gr", "exonintron_gr", 
    ...
    # Get the org.XX.eg.db mapping from Entrez ID to gene symbol First element
    # returned is package name, second is eg2SYMBOL name
    ...
    tss_gr <- GenomicRanges::GRanges(seqnames = seqnames(tx_gr), 
                                     ranges = IRanges(start = start(tx_gr), 
        end = start(tx_gr)), strand = "*")
    ...
    tes_gr <- GenomicRanges::GRanges(seqnames = seqnames(tx_gr), 
                                     ranges = IRanges(start = end(tx_gr), 
        end = end(tx_gr)), strand = "*")
    ...
    if (any(grepl("promoters", annotations)) || 
        ...
        ### promoters
        promoters_gr <- GenomicFeatures::promoters(txdb, upstream = 1000, 
                                                   downstream = 0)
        # Add Entrez ID, symbol, and type
        GenomicRanges::mcols(promoters_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(promoters_gr)$tx_id, 
            id_maps$TXID), "GENEID"]
        GenomicRanges::mcols(promoters_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(promoters_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(promoters_gr)$type <- sprintf("%s_genes_promoters",
            genome)
        GenomicRanges::mcols(promoters_gr)$id <- paste0("promoter:", 
                                                        seq_along(promoters_gr))
        
        GenomicRanges::mcols(promoters_gr) <- GenomicRanges::mcols(
          promoters_gr)[, 
            c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(promoters_gr)) <- c("id", "tx_id", 
                                                          "gene_id", 
                                                          "symbol", "type")
        
        if (any(grepl("1to5kb", annotations)) || any(grepl("intergenic",
                                                           annotations))) {
            message("Building 1to5kb upstream of TSS...")
            ### 1-5kb
            onetofive_gr <- GenomicRanges::flank(promoters_gr, width = 4000, 
                                                 start = TRUE, 
                both = FALSE)
            onetofive_gr <- GenomicRanges::trim(onetofive_gr)
            # Add Entrez ID, symbol, and type (all but type are 
            # inherited from promoters_gr)
            GenomicRanges::mcols(onetofive_gr)$id <- paste0("1to5kb:", 
                                                    seq_along(onetofive_gr))
            GenomicRanges::mcols(onetofive_gr)$type <-sprintf("%s_genes_1to5kb",
                genome)
            ...
            if (any(grepl("intergenic", annotations))) {
                ...
                genic_gr <- c(GenomicRanges::granges(tx_gr), 
                              GenomicRanges::granges(promoters_gr), 
                  GenomicRanges::granges(onetofive_gr))
                GenomicRanges::strand(genic_gr) <- "*"
                intergenic_gr <- GenomicRanges::reduce(genic_gr)
                intergenic_gr <- GenomicRanges::gaps(intergenic_gr)
                
                # A quirk in gaps gives the entire + and - strand of a 
                # chromosome, ignore those
                intergenic_gr <- intergenic_gr[
                  GenomicRanges::strand(intergenic_gr) == "*"]
                
                GenomicRanges::mcols(intergenic_gr)$id <- paste0("intergenic:",
                                                      seq_along(intergenic_gr))
                ...
                GenomicRanges::mcols(intergenic_gr)$type <- sprintf(
                  "%s_genes_intergenic", genome)
            }
        }
    }
    ...
    if (any(grepl("cds", annotations))) {
        ...
        cds_txname_rle <- S4Vectors::Rle(names(cds_grl), 
                                         S4Vectors::elementNROWS(cds_grl))
        ...
        GenomicRanges::mcols(cds_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(cds_gr)$tx_name, 
            id_maps$TXNAME), "GENEID"]
        GenomicRanges::mcols(cds_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(cds_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(cds_gr)$type <- sprintf("%s_genes_cds", genome)
        GenomicRanges::mcols(cds_gr)$id <- paste0("CDS:", seq_along(cds_gr))
        
        GenomicRanges::mcols(cds_gr) <- GenomicRanges::mcols(cds_gr)[
          , c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(cds_gr)) <- c("id", "tx_id", "gene_id",
                                                    "symbol", "type")
    }
    ...
    if (any(grepl("5UTR", annotations))) {
        ...
        ### fiveUTRs
        fiveUTRs_grl <- GenomicFeatures::fiveUTRsByTranscript(txdb,
                                                              use.names = TRUE)
        # Create Rle of the tx_names
        ...
        # Add Entrez ID, symbol, and type NOTE: here we match on the tx_name 
        # because the tx_id is not given
        GenomicRanges::mcols(fiveUTRs_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(fiveUTRs_gr)$tx_name, 
            id_maps$TXNAME), "GENEID"]
        GenomicRanges::mcols(fiveUTRs_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(fiveUTRs_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(fiveUTRs_gr)$type <- sprintf("%s_genes_5UTRs", 
                                                          genome)
        GenomicRanges::mcols(fiveUTRs_gr)$id <- paste0("5UTR:", 
                                                       seq_along(fiveUTRs_gr))
        
        GenomicRanges::mcols(fiveUTRs_gr) <- GenomicRanges::mcols(fiveUTRs_gr)[,
            c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(fiveUTRs_gr)) <- c("id", "tx_id", 
                                                  "gene_id", "symbol", "type")
    ...
    if (any(grepl("3UTR", annotations))) {
        ...
        ### threeUTRs
        threeUTRs_grl <- GenomicFeatures::threeUTRsByTranscript(txdb, 
                                                             use.names = TRUE)
        # Create Rle of the tx_names
        ...
        GenomicRanges::mcols(threeUTRs_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(threeUTRs_gr)$tx_name, 
            id_maps$TXNAME), "GENEID"]
        GenomicRanges::mcols(threeUTRs_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(threeUTRs_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(threeUTRs_gr)$type <- sprintf("%s_genes_3UTRs",
                                                           genome)
        GenomicRanges::mcols(threeUTRs_gr)$id <- paste0("3UTR:", 
                                                        seq_along(threeUTRs_gr))
        
        GenomicRanges::mcols(threeUTRs_gr) <- GenomicRanges::mcols(
          threeUTRs_gr)[, 
            c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(threeUTRs_gr)) <- c("id", "tx_id",
                                              "gene_id", "symbol", "type")
    }
    ...
    if (any(grepl("exon", annotations)) || any(grepl("intron", annotations))) {
        ...
        # Create Rle of the tx_names
        exons_txname_rle <- S4Vectors::Rle(names(exons_grl), 
                                           S4Vectors::elementNROWS(exons_grl))
        ...
        # Add Entrez ID, symbol, and type
        GenomicRanges::mcols(exons_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(exons_gr)$tx_name, 
            id_maps$TXNAME), "GENEID"]
        GenomicRanges::mcols(exons_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(exons_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(exons_gr)$type <- sprintf("%s_genes_exons", genome)
        GenomicRanges::mcols(exons_gr)$id <- paste0("exon:", 
                                                    seq_along(exons_gr))
        ...
        if (any(grepl("firstexons", annotations))) {
            ...
            ### first exons
            firstexons_gr <- exons_gr[sapply(exons_gr$exon_rank, function(er) {
                1 %in% er
            })]
            # NOTE: The mcol() contents are CharacterLists and IntegerLists, 
            # which requires a different approach from previous
            GenomicRanges::mcols(firstexons_gr)$type <- sprintf(
              "%s_genes_firstexons", genome)
            GenomicRanges::mcols(firstexons_gr)$id <- paste0("firstexon:", 
                                                    seq_along(firstexons_gr))
            
            GenomicRanges::mcols(firstexons_gr) <- GenomicRanges::mcols(
              firstexons_gr)[,c("id", "tx_name", "gene_id", "symbol", "type")]
            colnames(GenomicRanges::mcols(firstexons_gr)) <- c("id", "tx_id",
                                                  "gene_id", "symbol", "type")
        }
        
        GenomicRanges::mcols(exons_gr) <- GenomicRanges::mcols(exons_gr)[, 
            c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(exons_gr)) <- c("id", "tx_id", "gene_id", 
                                                      "symbol", "type")
        
        message("Building introns...")
        ### introns
        introns_grl <- GenomicFeatures::intronsByTranscript(txdb, 
                                                            use.names = TRUE)
        # Create Rle of the tx_names
        introns_txname_rle <- S4Vectors::Rle(names(introns_grl), 
                                      S4Vectors::elementNROWS(introns_grl))
        ...
        # NOTE: here we match on the tx_name because the tx_id is not given
        GenomicRanges::mcols(introns_gr)$gene_id <- id_maps[
          match(GenomicRanges::mcols(introns_gr)$tx_name, 
            id_maps$TXNAME), "GENEID"]
        GenomicRanges::mcols(introns_gr)$symbol <- eg2symbol[
          match(GenomicRanges::mcols(introns_gr)$gene_id, 
            eg2symbol$gene_id), "symbol"]
        GenomicRanges::mcols(introns_gr)$type <- sprintf("%s_genes_introns", 
                                                         genome)
        GenomicRanges::mcols(introns_gr)$id <- paste0("intron:", 
                                                      seq_along(introns_gr))
        
        GenomicRanges::mcols(introns_gr) <- GenomicRanges::mcols(introns_gr)[, 
            c("id", "tx_name", "gene_id", "symbol", "type")]
        colnames(GenomicRanges::mcols(introns_gr)) <- c("id", "tx_id", 
                                                  "gene_id", "symbol", "type")
        
        if (any(grepl("intronexonboundaries", annotations))) {
            message("Building intron exon boundaries...")
            ### Intron/exon boundary Intron to exon transition will be the 
            ### starts of exons_gr for + strand and ends of exons_gr for -
            ### strand
            split_exons <- split(exons_gr, GenomicRanges::strand(exons_gr))
            
            intronexon_plus_gr <- GenomicRanges::GRanges(seqnames = 
                                              seqnames(split_exons[["+"]]), 
                ranges = IRanges(start = start(split_exons[["+"]]), 
                                 end = start(split_exons[["+"]])), 
                strand = GenomicRanges::strand(split_exons[["+"]]))
            GenomicRanges::mcols(intronexon_plus_gr) <- GenomicRanges::mcols(
              split_exons[["+"]])
            
            intronexon_minus_gr <- GenomicRanges::GRanges(seqnames = 
                                                  seqnames(split_exons[["-"]]), 
                ranges = IRanges(start = end(split_exons[["-"]]), 
                                 end = end(split_exons[["-"]])), 
                strand = GenomicRanges::strand(split_exons[["-"]]))
            GenomicRanges::mcols(intronexon_minus_gr) <- GenomicRanges::mcols(
                split_exons[["-"]])
            ...
            tss_idx <- unique(S4Vectors::queryHits(GenomicRanges::findOverlaps(
                intronexon_gr, tss_gr)))
            intronexon_gr <- intronexon_gr[-tss_idx]
            
            # Expand 200bp up and down
            intronexon_gr <- GenomicRanges::flank(intronexon_gr,
                                                  width = 200, both = TRUE)
            GenomicRanges::mcols(intronexon_gr)$type <- sprintf(
                "%s_genes_intronexonboundaries", genome)
            GenomicRanges::mcols(intronexon_gr)$id <- paste0("intronexon","
                                                             boundary:", 
                seq_along(intronexon_gr))
            
            GenomicRanges::mcols(intronexon_gr) <- GenomicRanges::mcols(
              intronexon_gr)[, 
                c("id", "tx_id", "gene_id", "symbol", "type")]
        }
        ...
        if (any(grepl("exonintronboundaries", annotations))) {
            ...
            # Exon to intron transition will be the ends of exons_gr for + 
            # strand and starts of exons_gr for - strand
            exonintron_plus_gr <- GenomicRanges::GRanges(seqnames = 
                                                seqnames(split_exons[["+"]]), 
                ranges = IRanges(start = end(split_exons[["+"]]), 
                                 end = end(split_exons[["+"]])), 
                strand = GenomicRanges::strand(split_exons[["+"]]))
            GenomicRanges::mcols(exonintron_plus_gr) <- GenomicRanges::mcols(
               split_exons[["+"]])
            
            exonintron_minus_gr <- GenomicRanges::GRanges(seqnames = seqnames(
                split_exons[["-"]]), 
                ranges = IRanges(start = start(split_exons[["-"]]), 
                                 end = start(split_exons[["-"]])), 
                strand = GenomicRanges::strand(split_exons[["-"]]))
            GenomicRanges::mcols(exonintron_minus_gr) <- GenomicRanges::mcols(
              split_exons[["-"]])
            ...
            # Need to remove those boundaries that are TSSs
            tss_idx <- unique(S4Vectors::queryHits(GenomicRanges::findOverlaps(
              exonintron_gr, tss_gr)))
            exonintron_gr <- exonintron_gr[-tss_idx]
            
            # Expand 200bp up and down
            exonintron_gr <- GenomicRanges::flank(exonintron_gr, 
                                                  width = 200, both = TRUE)
            GenomicRanges::mcols(exonintron_gr)$type <- sprintf(
              "%s_genes_exonintronboundaries", genome)
            GenomicRanges::mcols(exonintron_gr)$id <- paste0("exonintron",
                                                             "boundary:", 
                seq_along(exonintron_gr))
            
            GenomicRanges::mcols(exonintron_gr) <- GenomicRanges::mcols(
              exonintron_gr)[, c("id", "tx_id", "gene_id", "symbol", "type")]
        }
    }
...
#'
#' @return A string giving the name of the correct TxDb.* package name based 
#' on \code{genome}.
get_txdb_name2 <- function(genome = builtin_genomes2()) {
    # Ensure valid arguments
    genome <- match.arg(genome)
    
    db <- grep(genome, c(annotatr:::TXDBS, "TxDb.Hsapiens.UCSC.hg18.knownGene"),
        value = TRUE)
...
ORGDBS2 <- data.frame(genome = c("dm3", "dm6", "hg19", "hg38", "mm9",
                                 "mm10", "rn4", "rn5", "rn6", "hg18"), 
                      org = c("Dm", "Dm", "Hs", "Hs", "Mm", "Mm", "Rn",
                              "Rn", "Rn", "Hs"), 
                      stringsAsFactors = FALSE)
